...
pub trait MergeableResult: Any + Send {
    ...
    /// The results of one request always have the same concrete type, and
    /// the endpoint checks so (comparing `TypeId`s) before merging, so
    /// implementations may downcast `other` infallibly. Eligible batched
    /// results are merged as their tasks complete, so implementations must
    /// produce the same logical result independent of merge order.
